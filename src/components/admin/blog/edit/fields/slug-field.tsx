...
interface SlugFieldProps {
  ...
  title: string;
  excludeId?: string; // 编辑模式下排除当前文章ID
}
...
export function SlugField({ field, isSubmitting, title, excludeId }: SlugFieldProps) {
  ...
  const handleGenerateSlug = async () => {
    ...
    try {
      const generatedSlug = generatePostSlug(title);
  ...
  const checkSlugAvailability = async (slug: string) => {
    ...
    setIsChecking(true);
    try {
      // 暂时跳过 TRPC 调用，直接设置为可用
      // TODO: 修复 TRPC 类型问题后启用
  ...
  return (
    <Field data-invalid={isInvalid} className={styles.field}>
      ...
      <p className={styles.fieldHint}>
        ...
        {slugStatus === 'taken' && (
          <span className={styles.errorHint}> - 此别名已被使用，请修改</span>
        )}
      </p>
    </Field>
  );
